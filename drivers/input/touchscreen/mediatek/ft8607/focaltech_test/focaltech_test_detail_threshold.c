...
{
	size_t const size_uchar = TX_NUM_MAX * RX_NUM_MAX * sizeof(unsigned char);
	size_t const size_int = TX_NUM_MAX * RX_NUM_MAX * sizeof(int);
	size_t const size_short = TX_NUM_MAX * RX_NUM_MAX * sizeof(short);

	TP_TEST_LOGI("start\n");

	g_stCfg_MCap_DetailThreshold.InvalidNode =
			(unsigned char (*)[RX_NUM_MAX]) fts_malloc(size_uchar);
	...
	g_stCfg_MCap_DetailThreshold.RawDataTest_Min =
			(int (*)[RX_NUM_MAX]) fts_malloc(size_int);
	...
	g_stCfg_MCap_DetailThreshold.SCapCbTest_ON_Max =
			(short (*)[RX_NUM_MAX]) fts_malloc(size_short);
...
//		TP_TEST_LOGD("GetPrivateProfileString = %d\n",  dividerPos);
...
/*
	...
	for (i = 0; i < g_ScreenSetParam.iUsedMaxTxNum; i++) {
		...
		for (j = 0; j<dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.TxShortAdvance[i][k] =
						(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_ForceTouch_SCapRawDataTest(char *strIniFile)
{
	...
	int k = 0, i = 0, j = 0;
	//////////////////OFF
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapRawDataTest_OFF_Min",
			"150", str, strIniFile);
	MinValue = fts_atoi(str);
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapRawDataTest_OFF_Max",
			"1000", str, strIniFile);
	MaxValue = fts_atoi(str);
	...
	for (i = 0; i < FORCETOUCH_ROW; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapRawDataTest_OFF_Max[i][j] =
					MaxValue;
		}
	}
	...
	for (i = 0; i < FORCETOUCH_ROW; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapRawDataTest_OFF_Min[i][j] =
					MinValue;
		}
	}
	...
	//////////////////ON
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapRawDataTest_ON_Min",
			"150", str, strIniFile);
	MinValue = fts_atoi(str);
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapRawDataTest_ON_Max",
			"1000", str, strIniFile);
	MaxValue = fts_atoi(str);
