...
static int zmc_memory_init(struct reserved_mem *rmem)
{
	...
	zmc_base = zmc_shrink_cma_range();
	if (zmc_base > rmem->base) {
		pr_info("ZONE_NORMAL [0x%lx:0x%lx] (0x%lx)\n",
				(unsigned long)rmem->base,
				(unsigned long)(zmc_base - rmem->base), (unsigned long)zmc_base);
		memblock_free(rmem->base, zmc_base - rmem->base);
	}
