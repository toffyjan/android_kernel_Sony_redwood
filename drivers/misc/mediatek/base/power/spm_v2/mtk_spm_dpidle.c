...
#if defined(CONFIG_MACH_MT6757) || defined(CONFIG_MACH_KIBOPLUS)
	if (pcmdesc->size != get_spm_fw_addr_2nd()) {
		spm_crit2("DP Error: pcmdesc->size = %d. Recover it to %d.\n", pcmdesc->size, get_spm_fw_addr_2nd());
		*(u16 *) &pcmdesc->size = get_spm_fw_addr_2nd();
		aee_kernel_warning("DP Error", "pcm size incorrect");
	}
#endif
