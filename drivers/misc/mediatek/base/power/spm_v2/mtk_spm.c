...
int spm_load_pcm_firmware(struct platform_device *pdev)
{
	...
	for (i = DYNA_LOAD_PCM_SUSPEND; i < DYNA_LOAD_PCM_MAX; i++) {
		...
		if (pdesc->addr_2nd) {
			...
			/* record it in global variable for external reference */
			spm_fw_addr_2nd = addr_2nd;
		}
...
u16 get_spm_fw_addr_2nd(void)
{
	return spm_fw_addr_2nd;
}
EXPORT_SYMBOL(get_spm_fw_addr_2nd);
