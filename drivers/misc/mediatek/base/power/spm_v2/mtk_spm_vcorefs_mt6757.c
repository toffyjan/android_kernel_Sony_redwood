...
char *spm_vcorefs_dump_dvfs_regs(char *p)
{
	if (p) {
		...
		p += sprintf(p, "SPM_IRQ_STA: 0x%x, PCM_FSM_STA=0x%x\n",
				spm_read(SPM_IRQ_STA), spm_read(PCM_FSM_STA));
	} else {
		...
		spm_vcorefs_info("SPM_RSV_CON: 0x%x, SPM_RSV_STA: 0x%x\n",
				spm_read(SPM_RSV_CON), spm_read(SPM_RSV_STA));
		spm_vcorefs_info("SPM_IRQ_STA: 0x%x, PCM_FSM_STA=0x%x\n",
				spm_read(SPM_IRQ_STA), spm_read(PCM_FSM_STA));
	}
...
int spm_vcorefs_set_opp(int opp, int vcore, int ddr, bool non_force)
{
	...
	/* check r15 for fw running */
	spm_r15 = spm_read(PCM_REG15_DATA);
	if (spm_r15 == 0)
		spm_vcorefs_err(
			"Invalid r15(0x%x) ptr:0x%x len:%u r11:0x%x sw_dbg:0x%x 2nd_adr=%u\n",
			spm_r15, spm_read(PCM_IM_PTR), spm_read(PCM_IM_LEN),
			spm_read(PCM_REG11_DATA), spm_read(SPM_SW_DEBUG),
			get_spm_fw_addr_2nd());
