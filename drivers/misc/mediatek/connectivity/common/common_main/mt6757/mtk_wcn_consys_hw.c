...
/* [BY57][Bug288] Enable Clock source type TCXO for MT6625L */
...
INT32 mtk_wcn_consys_co_clock_type(VOID)
{
	...
	/* [BY57][Bug288] Enable Clock source type TCXO for MT6625L */
	/* SM11: PDP, DP1, DP2 -> co-TSX mode, Others -> TCXO mode */
	/* SM21: PDP -> co-TSX mode, Others -> TCXO mode */
	hw_board_id = get_cei_hw_id();
	customer_project_id = get_cei_customer_project_id();

	if (((customer_project_id == CUSTOMER_PROJECT_SM11) && ((hw_board_id >= CEI_HW_EVT) && (hw_board_id < CEI_HW_DVT2)))
		|| ((customer_project_id == CUSTOMER_PROJECT_SM21) && (hw_board_id == CEI_SM21_HW_EVT))) {
		co_clock_type = 1;
		WMT_PLAT_INFO_FUNC("Keep clock type as co-TSX mode, co_clock_type:%d\n", co_clock_type);
	} else {
		co_clock_type = 0;
...
/*step1.PMIC ctrl*/
		...
		if (co_clock_type) {
			...
			WMT_PLAT_INFO_FUNC("co clock type(%d), TCXO mode\n", co_clock_type);
			/*if NOT co-clock: */
			...
			//[BY57][Bug971] Fixed issue DUT has higher suspend current when WCN clock type is TCXO
			WMT_PLAT_INFO_FUNC("switch VCN28 to HW control mode\n");
			//pmic_set_register_value(MT6351_PMIC_RG_VCN28_ON_CTRL, 0);
...
#else
			if (reg_VCN28) {
				WMT_PLAT_INFO_FUNC("Ready to enable VCN28 for TCXO mode\n");
...
/*step2.MTCMOS ctrl*/
		...
		if (co_clock_type) {
			...
			//[BY57][Bug971] Fixed issue DUT has higher suspend current when WCN clock type is TCXO
			WMT_PLAT_INFO_FUNC("setup VCN28 to HW control mode\n");
			//pmic_set_register_value(MT6351_PMIC_RG_VCN28_ON_CTRL, 0);
			pmic_set_register_value(MT6351_PMIC_RG_VCN28_ON_CTRL, 1);
