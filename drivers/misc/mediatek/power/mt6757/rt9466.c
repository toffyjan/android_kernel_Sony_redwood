...
static struct rt9466_desc rt9466_default_desc = {
	...
	.ieoc = 150,	/* mA */
	.safety_timer = 12,
	.ircmp_resistor = 0,//CEI comment, ori=80
	.ircmp_vclamp = 0,//CEI comment, ori=224
	.enable_te = true,
	.enable_wdt = true,
...
static int rt9466_irq_register(struct rt9466_info *info)
{
	...
	ret = request_threaded_irq(info->irq, NULL, rt9466_irq_handler,
		IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "rt9466_irq", info);
	if (ret < 0) {
...
#if 0 /* Mask IRQs that are releated to plug-in/out */
...
static int rt9466_init_setting(struct rt9466_info *info)
{
	...
	bool enable_safety_timer = false; //CEI comment, ori=True
...
//CEI comment start//
	for (i = 0; i < ARRAY_SIZE(rt9466_reg_addr)/9; i++) {
		...
		if(i < (ARRAY_SIZE(rt9466_reg_addr)/9) -1)
		{
			...
			battery_log(BAT_LOG_CRTI, "LE(K)=> [rt9466 reg@][0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x\n",
		...
		else if(i == ((ARRAY_SIZE(rt9466_reg_addr)/9) -1))
		{
			...
			battery_log(BAT_LOG_CRTI, "LE(K)=> [rt9466 reg@][0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x\n",
						rt9466_reg_addr[(i*9)+0], rt9466_reg_buf[0], rt9466_reg_addr[(i*9)+1], rt9466_reg_buf[1], rt9466_reg_addr[(i*9)+2], rt9466_reg_buf[2], 
						rt9466_reg_addr[(i*9)+3], rt9466_reg_buf[3], rt9466_reg_addr[(i*9)+4], rt9466_reg_buf[4], rt9466_reg_addr[(i*9)+5], rt9466_reg_buf[5],
