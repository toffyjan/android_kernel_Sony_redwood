...
//S : User debug log check - Display : {
#define DEBUG 0
//E : User debug log check - Display : }
...
int primary_display_dsi_vfp_change(int state)
{
	int ret = 0;
	struct cmdqRecStruct *handle = NULL;
	if(DEBUG) DISPMSG("[disp_lowpower]%s\n", __func__);
...
void _acquire_wrot_resource_nolock(enum CMDQ_EVENT_ENUM resourceEvent)
{
	...
	if(DEBUG) DISPINFO("[disp_lowpower]%s\n", __func__);
	if (use_wrot_sram())
		return;
	...
	if (acquireResult < 0) {
		...
		if(DEBUG) DISPMSG("share SRAM success\n");
...
void _primary_display_enable_mmsys_clk(void)
{
	...
	if(DEBUG) DISPCHECK("[LP]2.dpmanager path config[end]\n");


	if(DEBUG) DISPDBG("[LP]3.dpmgr path start[begin]\n");
	dpmgr_path_start(primary_get_dpmgr_handle(), CMDQ_DISABLE);

	if (primary_display_is_decouple_mode())
		dpmgr_path_start(primary_get_ovl2mem_handle(), CMDQ_DISABLE);

	if(DEBUG) DISPINFO("[LP]3.dpmgr path start[end]\n");
	if (dpmgr_path_is_busy(primary_get_dpmgr_handle()))
...
static int _primary_path_idlemgr_monitor_thread(void *data)
{
	...
	while (1) {
		...
		if(DEBUG) DISPINFO("[disp_lowpower]primary enter idle state\n");
