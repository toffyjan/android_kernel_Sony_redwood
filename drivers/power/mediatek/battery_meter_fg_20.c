...
int get_car_tune_value(void)
{
	...
	static int car_tune_val = 112; // AP meseaured
#elif defined(CONFIG_BUILD_TEAK) // teak, sync redwood first
	static int car_tune_val = 112;
#else // hinoki
	static int car_tune_val = 112; // PQ meseaured
#endif
...
#define SUSPEND_CURRENT_CHECK_THRESHOLD 150	/* 15mA, ori = 10mA */
#endif
...
#if defined(CONFIG_BUILD_TEAK) || defined(CONFIG_BUILD_TEAK) // redwood and teak
			g_fg_battery_id = id;
			if (id == 0)
			{
				mdelay(200);
...
#if defined(CONFIG_BUILD_REDWOOD) || defined(CONFIG_BUILD_TEAK) //redwood and teak
...
void get_profile_id_and_volt(int *batt_id, int *batt_id_volt)
{
	...
	*batt_id_volt = 0;
	...
	if (ret != 0)
		...
		bm_print(BMLOG_INFO_LEVEL, "[get_profile_id_and_volt]CEI(K)=> id_volt = %d\n", id_volt);
...
//CEI comment start//
extern const int g_charge_full_design; //mAh
...
//CEI comments, aging fator start
		case FG_DAEMON_CMD_SET_AGING_FACTOR:
		{
			...
			bm_print(BMLOG_INFO_LEVEL, "[fg_res] aging_factor_tmp = %d\n", aging_factor_tmp);
			gFG_aging_factor_1 = aging_factor_tmp;
//CEI comment start//
			if( (aging_factor_tmp >= 0) && (aging_factor_tmp <= 100) )
			{
				int tmp_age_mtkcalc = 0;
				tmp_age_mtkcalc = (aging_factor_tmp * g_Q_MAX_POS_25[g_fg_battery_id]) / 100;
				if (tmp_age_mtkcalc > g_charge_full_design)
					tmp_age_mtkcalc = g_charge_full_design;

				g_age_mtkcalc = tmp_age_mtkcalc;
				g_age_counter = g_age_counter + 1;
				bm_print(BMLOG_INFO_LEVEL, "[AGING] CEI(K)=> g_Q_MAX_POS_25[%d]=%d, aging_factor_tmp=%d, g_age_mtkcalc=%d, g_age_counter=%d\n",
					g_fg_battery_id, g_Q_MAX_POS_25[g_fg_battery_id], aging_factor_tmp, g_age_mtkcalc, g_age_counter);
			}
//CEI comment end//
		}
		break;
//CEI comments, aging fator  end
