...
int g_battery_thermal_throttling_flag = 1;//CEI comments, safty timer, ori=3
...
#if defined(CONFIG_BUILD_REDWOOD) // redwood
const int g_charge_full_design = 2700; //mAh
#elif defined(CONFIG_BUILD_TEAK) // teak
const int g_charge_full_design = 3430; //mAh
#else // hinoki
const int g_charge_full_design = 2300; //mAh
#endif
int g_age_counter = 0;
int g_age_mtkcalc = 0;
...
static ssize_t show_age_testadd(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	g_age_mtkcalc = (random_value % (g_charge_full_design - 99)) + 100; // 100 ~ g_charge_full_design
